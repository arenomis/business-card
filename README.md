...
**Форма / копия заявителю:** при Resend + тестовом `onboarding@resend.dev` копия уходит через **SMTP в том же запросе**, что и ответ API (с таймаутом, чтобы форма не висела бесконечно). На проде для копии стабильнее **Brevo** или **подтверждённый домен в Resend**.
